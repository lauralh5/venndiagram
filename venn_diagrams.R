  ...
  theme = shinytheme("sandstone"),
   ...
   downloadButton("image2", "Download Venn Diagram as image")
    ...
    # Show a plot of the generated distribution
      mainPanel(
        h2("Venn Diagram for 2 data sets"),
        plotOutput("venn2")
      )
   ...
   sidebarLayout(
     ...
     # Show a plot of the generated distribution
     mainPanel(
       h2("Venn Diagram for 3 data sets"),
       plotOutput("venn3")
     )
  )
...
#  set1=reactive(as.vector(unlist(strsplit(x = input$dataset1, split = "\n")))))
  ...
  #flag = 2
  #flag = reactive(input$botones)
#  reactive (if (input$dataset1!="") {
 #   flag = flag+1
  #})
#  if (reactive(input$dataset2)==TRUE) {
 #   flag = flag+1
  #}
#  if (reactive(input$dataset3)==TRUE) {
 #   flag = flag+1
  #}
    ...
    output$venn3 = renderPlot({draw.triple.venn(length(a2()), length(b2()), length(d()), length(ab2()), length(bd()), length(ad()), length(abd()), 
                                                fill = c("green", "blue", "yellow"), category = c(label3(), label4(), label5()))})
    output$image2 <- downloadHandler(
      filename = "VennDiagram.png",
      content = function(file) {
        png(file)
        print(draw.pairwise.venn(length(a()), length(b()), length(ab()), fill = c("green", "blue"), category = c(label1(), label2())))
        dev.off()
      }
    )
